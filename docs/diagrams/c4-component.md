...
flowchart TB
    ...
    LLMNode -->|"fallback inference"| FallbackLLM
...
| `get_spending_report` | Получить агрегированный отчёт за период | При запросе отчёта |
| `set_limit` | Установить лимит по категории и периоду | При явном запросе лимита |
| `list_limits` | Показать все установленные лимиты | При запросе списка лимитов |
| `check_limit_violations` | Проверить нарушения лимитов | При анализе трат |
| `categorize_transaction` | Определить категорию по merchant | При нечёткой категоризации |
| `recategorize_all_transactions` | Пересчитать категории всех транзакций | Когда категории пустые/nan |
| `parse_transactions_from_text` | Извлечь транзакции из свободного текста | Когда пользователь пишет расходы текстом |
...
| `generate_refund_letter` | Составить заявление на возврат | При запросе письма о возврате |
